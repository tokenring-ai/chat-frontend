...
export default function App() {
    ...
    return (
      <div className="flex items-center justify-center h-dvh text-lg text-muted bg-primary">
        <div className="flex flex-col items-center gap-4">
          <div className="w-12 h-12 border-4 border-primary border-t-indigo-500 rounded-full animate-spin shadow-lg shadow-indigo-500/20"/>
  ...
  if (agents.error || agentTypes.error || workflows.error) {
    ...
    return (
      <div className="flex items-center justify-center h-dvh text-lg text-muted bg-primary p-4">
        <div className="flex flex-col items-center gap-4 max-w-md text-center">
          <div className="w-12 h-12 rounded-full bg-red-500/10 flex items-center justify-center shadow-lg shadow-red-500/20">
            <svg className="w-6 h-6 text-red-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
            </svg>
          </div>
          <div>
            <p className="text-lg font-semibold text-primary mb-2">Failed to Load</p>
            <p className="text-sm text-muted mb-4">{errorMessage}</p>
          </div>
          <button
            onClick={() => window.location.reload()}
            className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg transition-colors focus-ring font-medium flex items-center gap-2 shadow-lg shadow-indigo-600/20"
          >
    ...
    <SidebarProvider>
      <ChatInputProvider>
        <ErrorBoundary>
          ...
          {showLoadingBar && (
            <div className="fixed top-0 left-0 right-0 h-1 bg-linear-to-r from-cyan-500 via-indigo-500 to-indigo-600 z-[100]"/>
          )}
                  ...
                  <Routes>
                    <Route path="/agent/:agentId/*" element={
                      currentAgentId && agents.data && !agents.data.find(a => a.id === currentAgentId)
                        ? (
                          <div className="flex flex-col items-center justify-center h-full text-muted p-4 text-center max-w-md">
                            <div className="mb-6">
                              <div className="w-12 h-12 rounded-full bg-amber-500/10 flex items-center justify-center mx-auto mb-4 shadow-lg shadow-amber-500/20">
                                <svg className="w-6 h-6 text-amber-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                                        d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/>
                                </svg>
                              </div>
                              <p className="text-lg font-semibold text-primary mb-2">Agent Not Found</p>
                              <p className="text-sm text-muted">
                                ...
                                could not be found. It may have been stopped or removed.
                              </p>
                            </div>
                            <button
                              onClick={() => {
                              ...
                              className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-400 text-white rounded-lg transition-colors focus-ring font-medium flex items-center gap-2 shadow-lg shadow-indigo-600/20"
