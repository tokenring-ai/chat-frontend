        ...
        >
                      ...
                      return (
                        ...
                        >
                          <td className="pl-2 pr-4 py-2">
                            <button
                              ...
                              className={cn(
                                ...
                                isInChat ? "border-indigo-500 bg-indigo-500 shadow-sm" : "border-primary hover:border-muted",
                              )}
                          ...
                          <td className="px-2 py-2">
                            <div
                              className={cn("flex items-center gap-2 font-medium", isSelected ? "text-indigo-400" : isDir ? "text-primary" : "text-primary")}
                            >
                              {getFileIcon(file, isDir)}
                              <span className={cn(uploadingFiles.includes(displayName) && "text-indigo-400")}>{displayName}</span>
              ...
              {selectedFile ? (
                <>
                  <div className="p-4 border-b border-primary flex items-start gap-3">
                    <div className="w-10 h-10 rounded-lg bg-tertiary flex items-center justify-center shrink-0">{getFileIcon(selectedFile, false, 24)}</div>
          ...
          {selectedFile && (
            <div style={{ height: `${previewHeight}px` }} className="bg-tertiary border-t border-primary flex flex-col shrink-0 relative">
              <div onMouseDown={handleMouseDown} className="absolute top-0 left-0 right-0 h-1 cursor-ns-resize hover:bg-indigo-500/50 transition-colors z-10" />
              ...
              <div className="flex-1 overflow-auto custom-scrollbar bg-primary relative">
                {fileContent.data ? (
                  selectedFile.endsWith(".md") ? (
                    <MarkdownEditor content={fileContent.data.content ?? ""} onContentChange={setEditorContent} />
                  ) : (
                    <CodeEditor file={selectedFile} content={fileContent.data.content ?? ""} onContentChange={setEditorContent} />
                  )
                ) : (
