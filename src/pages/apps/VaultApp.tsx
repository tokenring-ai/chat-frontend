    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-primary">
          ...
          <button onClick={onClose} className="p-1.5 text-muted hover:text-primary transition-colors rounded-md cursor-pointer focus-ring">
            <X className="w-3.5 h-3.5"/>
          </button>
        </div>
        ...
        <div className="px-4 py-3 space-y-2">
          <p className="text-2xs text-muted">Paste <span className="font-mono">KEY=value</span> pairs, one per line. Lines starting with <span className="font-mono">#</span> are ignored.</p>
          <textarea
            autoFocus
            value={text}
        ...
        {preview.length > 0 && (
          <div className="mx-4 mb-3 bg-input border border-primary rounded-lg overflow-hidden">
            <p className="px-3 py-1.5 text-2xs text-muted border-b border-primary">{preview.length} key{preview.length !== 1 ? 's' : ''} detected</p>
          ...
          {filtered.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-10 gap-2 text-center px-4">
              ...
              <p className="text-xs text-muted">
                {search ? `No keys matching "${search}"` : 'No keys stored'}
              </p>
            </div>
          ) : (
            filtered.map(k => (
