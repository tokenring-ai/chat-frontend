...
function GeneratePanel({ agentId, onGenerated }: { agentId: string | null; onGenerated: () => void }) {
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex gap-2">
            {aspectOptions.map(opt => (
              ...
              >
                <div
                  className={`border-2 rounded-sm ${aspectRatio === opt.value ? "border-indigo-400" : "border-current"} ${
      ...
      <div className="flex-1 overflow-y-auto p-2">
        {loading && images.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-10 gap-3 text-center px-4">
            ...
            <p className="text-sm text-muted">{search ? `No images matching "${search}"` : "No images yet"}</p>
          </div>
        ) : (
          <div className="grid grid-cols-2 gap-2">
            {images.map(img => (
              <ImageThumbnail key={img.filename} image={img} selected={selectedImage?.filename === img.filename} onClick={() => onSelect(img)} />
            ))}
          </div>
        )}
      </div>
...
function RightPanel({
  ...
  if (selectedImage) {
    return <ImageViewer image={selectedImage} onWorkOnImage={onWorkOnImage} onClose={onClearSelection} />;
  }
  return <GeneratePanel agentId={agentId} onGenerated={onGenerated} />;
}
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (cancelled) return;
        const preferred = types.find(t => ["media", "image", "imageGeneration"].includes(t.type)) ?? types[0];
        if (!preferred) {
