...
function EventChip({ event, onClick, compact = false }: { event: CalendarEvent; onClick: () => void; compact?: boolean }) {
  ...
  return (
    ...
    >
      ...
      <span className="truncate">{compact ? '' : (event.startTime ? `${event.startTime} ` : '')}{event.title}</span>
    </button>
  );
}
    ...
    <div className="flex flex-col flex-1 min-h-0">
      ...
      <div className="grid grid-cols-7 border-b border-primary">
        {WEEKDAYS_SHORT.map(d => (
          <div key={d} className="py-2 text-center text-2xs font-semibold text-muted uppercase tracking-wider">{d}</div>
        ))}
      </div>
      {/* Date grid */}
        ...
        {cells.map((cell, i) => {
          ...
          return (
            <div
              ...
              aria-label={`${cell.toDateString()}${dayEvents.length ? `, ${dayEvents.length} events` : ''}`}
            >
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto relative">
        <div className="flex" style={{ height: `${24 * HOUR_H}px` }}>
          ...
          <div className="w-14 shrink-0 relative">
            {HOURS.map(h => (
              <div key={h} className="absolute left-0 right-0 flex items-start justify-end pr-2 text-2xs text-muted"
...
function DayView({
  ...
  const dayEvents = useMemo(() =>
    events.filter(ev => ev.date === dayKey && ev.startTime),
    [events, dayKey]);

  const allDay = useMemo(() =>
    events.filter(ev => ev.date === dayKey && (ev.allDay || !ev.startTime)),
    [events, dayKey]);
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-primary shrink-0">
        ...
        <div>
          <div className={cn('text-sm font-semibold', isToday ? 'text-sky-500' : 'text-primary')}>
            {WEEKDAYS_SHORT[date.getDay()]}
          </div>
          <div className="text-2xs text-muted">{MONTHS[date.getMonth()]} {date.getFullYear()}</div>
        </div>
        {allDay.length > 0 && (
          <div className="ml-4 flex gap-1 flex-wrap">
            {allDay.map(ev => <EventChip key={ev.id} event={ev} onClick={() => onEventClick(ev)} compact />)}
          </div>
        )}
      </div>
            ...
            {dayEvents.map(ev => {
              ...
              return (
                ...
                >
                  ...
                  {ev.startTime && <div className="text-white/80 text-2xs">{ev.startTime}{ev.endTime ? ` – ${ev.endTime}` : ''}</div>}
                </button>
              );
            })}
          </div>
...
function EventModal({ event, defaultDate, defaultHour, onClose, onSave, onDelete, onRun, running }: EventModalProps) {
  ...
  const isNew = !event;
  const defaultTime = defaultHour != null
  ...
  const handleSave = () => {
    if (!title.trim()) { toastManager.error('Title is required'); return; }
    if (type === 'workflow' && !workflowKey) { toastManager.error('Select a workflow'); return; }
    if (type === 'agent' && !agentType) { toastManager.error('Select an agent type'); return; }
    onSave({
      ...
      allDay,
      type,
      agentType: type === 'agent' ? agentType : undefined,
      workflowKey: type === 'workflow' ? workflowKey : undefined,
      color,
    });
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div
        className="bg-primary border border-primary rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden"
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 pt-5 pb-0">
          <h2 className="text-base font-bold text-primary">{isNew ? 'New Scheduled Event' : 'Edit Event'}</h2>
          <button onClick={onClose} className="p-1 rounded-lg hover:bg-hover transition-colors text-muted hover:text-primary cursor-pointer" aria-label="Close">
            <X size={18} />
          </button>
        </div>
        ...
        <div className="px-5 py-4 space-y-4">
              ...
              <button
                ...
                className={cn(
                  ...
                  type === 'workflow'
                    ...
                    : 'border-primary text-muted hover:border-sky-500/40 hover:text-primary',
                )}
              >
          ...
          {type === 'workflow' ? (
            <div>
              ...
              {workflows.isLoading ? (
                <div className="flex items-center gap-2 text-muted text-xs py-2"><Loader2 size={14} className="animate-spin" /> Loading…</div>
              ) : (workflows.data?.length ?? 0) === 0 ? (
                <p className="text-xs text-muted py-1">No workflows defined in <code>.tokenring/workflows/</code></p>
              ) : (
                <select
                  value={workflowKey}
            ...
            <div>
              ...
              {agentTypes.isLoading ? (
                ...
                >
                  ...
                  {agentTypes.data?.map(t => <option key={t.type} value={t.type}>{t.displayName}</option>)}
                </select>
              )}
            </div>
          )}
        ...
        <div className="px-5 pb-5 flex items-center gap-2">
          ...
          <div className="flex-1" />
          <button onClick={onClose} className="px-4 py-2 text-xs font-semibold text-muted hover:text-primary transition-colors cursor-pointer">
            Cancel
          </button>
          <button
            onClick={handleSave}
...
export default function CalendarApp() {
  ...
  const handleRunEvent = useCallback(async (ev: CalendarEvent) => {
    setRunning(true);
    try {
